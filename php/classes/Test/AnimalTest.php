...
Class AnimalTest extends LostPawsTest{
	...
	public function testGetAnimalByAnimalSpecies() : void {
		...
		$animal->insert($this->getPDO());
		// grab the data from mySQL and enforce that the fields match expectations
		$results = Animal::getAnimalByAnimalSpecies($this->getPDO(), $animal->getAnimalSpecies());
		$this->assertEquals($numRows + 1, $this->getConnection()->getRowCount("animal"));
		$this->assertCount(1, $results);
		//enforce that no other objects are bleeding into the test
		$this->assertContainsOnlyInstancesOf("Jisbell347\LostPaws\Animal", $results);
	...
	/**
	 * test grabbing an animal by an animal Species that does not exist
	 **/
	...
	/**
	 * test grabbing an animal by status
	 **/
	public function testGetAnimalByAnimalStatus() : void {
		// count the number of rows and save it for later
		$numRows = $this->getConnection()->getRowCount("animal");
		//create a new Animal and insert into mySQL
		$animalId = generateUuidV4();
		...
		$animal->insert($this->getPDO());
		// grab the data from mySQL and enforce that the fields match expectations
		$results = Animal::getAnimalByAnimalStatus($this->getPDO(), $animal->getAnimalStatus());
		$this->assertEquals($numRows + 1, $this->getConnection()->getRowCount("animal"));
		$this->assertCount(1, $results);
		//enforce that no other objects are bleeding into the test
		$this->assertContainsOnlyInstancesOf("Jisbell347\LostPaws\Animal", $results);
	...
	/**
	 * test grabbing an animal by an animal Status that does not exist
	 **/
	public function testGetInvalidAnimalByAnimalStatus() : void {
		//grab an animal by status that does not exist for an animal
		$animal = Animal::getAnimalByAnimalStatus($this->getPDO(),"Intoxicated");
		$this->assertCount(0, $animal);
	}
	/**
